...
/*  To compile this example from within the example/ folder along with the
    library itself, use:
      g++ -o test iForestASDStreamingScikitMultiFlow.cpp $(ls ../src | grep ^[^R] | grep cpp | perl \
   ...
   Then compile this single file and link to the shared library:
     g++ -o test iForestASDStreamingScikitMultiFlow.cpp -lisotree -std=c++11
   ...
   the root folder:
     g++ -o test example/iForestASDStreamingScikitMultiFlow.cpp -std=c++11 -I./include -l:libisotree.so -L./build -Wl,-rpath,./build
